...
use osmformat::mod_OSMPBF as osmformat;
use osmformat::mod_OSMPBF::{PrimitiveGroup, PrimitiveBlock};
use std::slice;
use std::iter::Chain;
use std::iter::Map;
...
impl<'a> Iterator for Relations<'a> {
    ...
    fn next(&mut self) -> Option<Relation> {
        use osmformat::mod_OSMPBF::mod_Relation::MemberType::{NODE, WAY, RELATION};
